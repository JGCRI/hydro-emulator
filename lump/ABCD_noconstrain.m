function [Qsim,Ea, G, S,RE,DR,base,XS,RAIN,SNOW,SNM]=ABCD_noconstrain(pars,P,PET,Inv,TMIN)
% Reference:
...
% Qsim are Ea are simulated streamflow and actual evapotranspiration respectively
...
Qsim=zeros(size(P));%simulated runoff
...
XS=zeros(size(P));%accumulated snow water equivalent
RAIN=zeros(size(P));
SNOW=zeros(size(P));
...
Qsim(:,1)=repmat(Inv(1),size(Qsim,1),1);% initial steamflow
