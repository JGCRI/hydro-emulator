% This is the main program that calibrates the lumped schemd of the hydrological emulator (HE) 
...
% the Calibration\lump is located.
% cd .\Calibration\lump;
...
for i=1:size(files,1)      
    ...
    PET=nanmean(WATCH_basin_grid.PET(:,1:nmon));PET0=repmat(PET,1,2); 
    P=nanmean(WATCH_basin_grid.P(:,1:nmon));P0=repmat(P,1,2);
    TMIN=nanmean(WATCH_basin_grid.TMIN(:,1:nmon));TMIN0=repmat(TMIN,1,2);
    Robs=nanmean(WATCH_basin_grid.Robs(:,1:nmon));Robs0=repmat(Robs,1,2);%Robs:observed runoff
    obsbfi=nanmean(WATCH_basin_grid.obsbfi);%baseflow index
    ...
    [pars]=Calibration_lump(PET,P,TMIN,Robs,obsbfi);
    datestr(now)
    ...
    data2=[P',Robs', Rsim',Ea',PET', G', S',RE',DR',base',XS',RAIN',SNOW',SNM'];%Rsim: simulated runoff, see other variables in abcd.m

    KGEval(basin_id,:)=[basin_id KGE(Robs',Rsim')];
    bsn_data(:,:,basin_id)=data2;

    simbfi=nanmean(base./Rsim,2);
    name=['Cal_lump_basin_' num2str(basin_id) '.mat'];
    lump_cal=struct('basin_name',basin_name,'basin_id',basin_id,'pars',pars,'KGE',KGEval(basin_id,2),'P',P,'PET',PET,...
...
name2='KGE_Cal_lump_glb.mat';
name3='bsn_data_Cal_lump_glb.mat';
save(name2,'KGEval'); 
save(name3,'bsn_data');
fclose all;
