% This function is to get the calibrated parameters (a,b,c,d,m) for the lumped scheme 
...
function [KGE_output,bsn_output,Rsim,Ea, G, S,RE,DR,base,XS,RAIN,SNOW,SNM] = Validation_lump(PET,P,TMIN,Robs,lump_cal)  
    %KGE_output: KGE value during the validation period
%   %bsn_output: output from the lumped scheme of the hydrological emulator during the validation period
%   %lump_cal: the basin-specific structure arragy output from the calibration process
    ...
    Inv0=[nanmean(lump_cal.Rsim(:,end-35:12:end)) nanmean(lump_cal.SM(:,end-35:12:end)) nanmean(lump_cal.GW(:,end-35:12:end))];
    ...
    bsn_output=[P',Robs', Rsim',Ea',PET', G', S',RE',DR',base',XS',RAIN',SNOW',SNM'];%see meanings of the variables in abcd.m
    KGE_output= KGE(Robs',Rsim');
