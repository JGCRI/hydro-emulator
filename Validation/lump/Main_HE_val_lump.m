% This is the main program that validates the capability of the lumped schemd of the 
...
% the Valiation\lump is located.
% cd .\Validation\lump;

files=dir('..\..\WATCH_basin_data\WATCH_basin_*_grid.mat');files=struct2cell(files);files=files(1,:)';%basin-specific input data
files2=dir('..\..\Calibration\lump\Cal_lump_basin_*.mat');files2=struct2cell(files2);files2=files2(1,:)';%output from calibration
...
for i=1:size(files,1)      
    ...
    file2=files2{tmpidx};
    lump_cal=load(['..\..\Calibration\lump\' file2]); 
    lump_cal=lump_cal.lump_cal;   

    %take the period of 1991-2010 as the validation period in our example    
    PET=nanmean(WATCH_basin_grid.PET(:,end-nmon+1:end));%potential evapotranpiration
    P=nanmean(WATCH_basin_grid.P(:,end-nmon+1:end));%precipitation
    TMIN=nanmean(WATCH_basin_grid.TMIN(:,end-nmon+1:end));%minimum air temperature
    Robs=nanmean(WATCH_basin_grid.Robs(:,end-nmon+1:end));%Robs:observed runoff
    obsbfi=nanmean(WATCH_basin_grid.obsbfi);    
    basin_name=WATCH_basin_grid.basin_name;
    basin_id=WATCH_basin_grid.basin_id;    
    grids=WATCH_basin_grid.grids;
    
    [KGE_output,bsn_output,Rsim,Ea, G, S,RE,DR,base,XS,RAIN,SNOW,SNM] = Validation_lump(PET,P,TMIN,Robs,lump_cal);  
   
    KGEval(basin_id,:)=[basin_id KGE_output];
    bsn_data(:,:,basin_id)=bsn_output;

    simbfi=nanmean(base./Rsim,2);
    name=['Val_lump_basin_' num2str(basin_id) '.mat'];
    lump_val=struct('basin_name',basin_name,'basin_id',basin_id,'grids',grids,'KGE',KGEval(basin_id,2),'P',P,...
...
name2='KGE_Val_lump_glb.mat';
name3='bsn_data_Val_lump_glb.mat';
save(name2,'KGEval'); 
save(name3,'bsn_data');
fclose all;
